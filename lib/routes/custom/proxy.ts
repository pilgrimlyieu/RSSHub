...
async function handler(ctx): Promise<Data> {
    ...
    const items = (feed.items || []).map((item: any) => {
        const enclosure: { url: string; type?: string; length?: string | number } | undefined = item.enclosure;
        const enclosureLength = enclosure?.length;
        ...
        return {
            ...
            ...(enclosure?.url && {
                ...
                enclosure_length: enclosureLength ? Number(enclosureLength) : undefined,
            }),
        };
    });
